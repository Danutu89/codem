...
export const makeGitManager = Effect.gen(function* () {
  ...
  const resolveCommitAndBranchSuggestion = (input: {
    ...
    /** Explicit provider override forwarded to text generation. */
  ...
  const runPrStep = (cwd: string, fallbackBranch: string | null, provider?: ProviderKind | undefined) =>
    Effect.gen(function* () {
  ...
  const runFeatureBranchStep = (cwd: string, branch: string | null, commitMessage?: string, provider?: ProviderKind | undefined) =>
    Effect.gen(function* () {
