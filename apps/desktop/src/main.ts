...
import { app, BrowserWindow, dialog, ipcMain, Menu, net, nativeImage, protocol, session, shell } from "electron";
...
const APP_DISPLAY_NAME = isDevelopment ? "Codem (Dev)" : "Codem";
const APP_USER_MODEL_ID = "com.codem.app";
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Codem failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function registerDesktopProtocol(): void {
  ...
  protocol.handle(DESKTOP_SCHEME, (request) => {
    ...
    // Hashed assets (e.g. index-B7Nl_oFq.js) are immutable; HTML must always revalidate.
...
app
  ...
  .then(async () => {
    ...
    // Clear cached web content so updates are always picked up.
